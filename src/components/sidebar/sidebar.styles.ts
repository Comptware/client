...
export const SidebarWrapper = tv({
  ...
  base: "bg-background transition-transform fixed -translate-x-full w-64 shrink-0 z-[202] overflow-y-auto border-r border-divider flex-col py-6 px-3 md:ml-0 md:flex md:static h-full md:translate-x-0 ",
  variants: {
    collapsed: {
...
export const Header = tv({
  base: "flex gap-8 items-center px-6 md:hidden",
});

export const Body = tv({
  base: "flex flex-col gap-6 md:mt-0 mt-9 px-2",
});
